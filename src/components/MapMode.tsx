...
const fetchWithRetry = async (url: string, retries = 3, delay = 800) => { if (typeof navigator !== 'undefined' && !navigator.onLine) { throw new Error("No Internet Connection"); } for (let i = 0; i < retries; i++) { try { const controller = new AbortController(); const id = setTimeout(() => controller.abort(), 5000); const res = await fetch(url, { signal: controller.signal }); clearTimeout(id); if (!res.ok) { if(res.status === 429 || res.status >= 500) { await new Promise(r => setTimeout(r, delay * 2 * (i + 1))); continue; } throw new Error(`HTTP ${res.status}`); } return await res.json(); } catch (e) { if (i === retries - 1) throw e; await new Promise(r => setTimeout(r, delay * (i + 1))); } } return null; };
