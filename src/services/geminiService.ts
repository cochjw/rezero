...
// Always initialize with import.meta.env.VITE_API_KEY directly as per guidelines
const ai = new GoogleGenAI({ apiKey: import.meta.env.VITE_API_KEY as string });

export const generateRecipe = async (ingredients: Ingredient[]): Promise<string> => {
  if (!import.meta.env.VITE_API_KEY) return "API Key가 설정되지 않았습니다.";
...
export const generateIngredientImage = async (): Promise<string | null> => {
  return null;
};
